...
function moveStar() {
	...
	if (counter * defaultLenght >= straightLength) {
		/* we've hit the end! */
		direction = false;
	} else if (counter <= 0) {
	...
	stars[0].setAttribute("transform","translate("+ (svg[0].getPointAtLength(counter * defaultLenght).x -0)  + "," + (svg[0].getPointAtLength(counter * defaultLenght).y -0) + ")");
...
function resetSVG() {
	...
	svg = svgContainer.getElementsByTagNameNS(ns, "path");

	var pathD = arrayPath[Math.floor(Math.random() * arrayPath.length)];
	newpath = document.createElementNS('http://www.w3.org/2000/svg',"path"); 
	circle = document.createElementNS('http://www.w3.org/2000/svg',"circle"); 
	circle.setAttributeNS(null, 'r', '15'); circle.setAttributeNS(null, 'cy', '-30'); 
	newpath.setAttributeNS(null, "d", pathD);
	$('#outerWrapper svg').empty();
	$('#outerWrapper svg').append(circle);
	$('#outerWrapper svg').append(newpath);
	straightLength = svg[0].getTotalLength();
	$('#kill').text(trung);
}
function die () {
	resetSVG();
